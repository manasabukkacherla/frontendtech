    ...
    const attemptUpload = async (retries = 2): Promise<any> => {
      try {
        return await axios.post(
          'https://backend-sgxi.onrender.com/api/property-media/upload',
          formData,
          {
            headers: {
...
export const deletePropertyMediaFromS3 = async (
  ...
  try {
    const response = await axios.delete(`https://backend-sgxi.onrender.com/api/property-media/${propertyType}/${propertyId}/${mediaId}`);
...
export const getPropertyMediaFromS3 = async (
  ...
  try {
    const response = await axios.get(`https://backend-sgxi.onrender.com/api/property-media/${propertyType}/${propertyId}`);
